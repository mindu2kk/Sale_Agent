...
class PriceAccuracyThresholds(BaseModel):
    ...
    def major_must_exceed_minor(cls, v, values):
        ...
        if v <= minor:
            raise ValueError(
                f"major_threshold_percent ({v}) must be > minor_threshold_percent ({minor})"
            )
        return v

    @validator("critical_threshold_percent")
    def critical_must_exceed_major(cls, v, values):
        major = values.get("major_threshold_percent", 0)
        if v <= major:
            raise ValueError(
                f"critical_threshold_percent ({v}) must be > major_threshold_percent ({major})"
            )
        return v
...
class TopicRelevanceThresholds(BaseModel):
    ...
    @validator("major_coverage_threshold")
    def major_must_be_below_minor(cls, v, values):
        minor = values.get("minor_coverage_threshold", 1.0)
        if v >= minor:
            raise ValueError(
                f"major_coverage_threshold ({v}) must be < minor_coverage_threshold ({minor})"
            )
        return v

    @validator("critical_coverage_threshold")
    def critical_must_be_below_major(cls, v, values):
        major = values.get("major_coverage_threshold", 1.0)
        if v >= major:
            raise ValueError(
                f"critical_coverage_threshold ({v}) must be < major_coverage_threshold ({major})"
            )
        return v
...
class VerificationThresholdsConfig(BaseModel):
    ...
    @validator("verification_weights")
