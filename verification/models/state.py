...
class WorkflowStateValidator(BaseModel):
    ...
    **Validates: Requirements 7.1, 7.2** - State management và observability
    ...
    def validate_end_time(cls, v, values):
        ...
        try:
            start_dt = datetime.fromisoformat(values.get('start_time', '').replace('Z', '+00:00'))
            ...
            if "end_time must be after start_time" in str(e):
                raise e
            raise ValueError("end_time must be valid ISO format timestamp")
    ...
    def validate_workflow_status_transitions(cls, v, values):
        """Validate logical workflow status transitions"""
        # Define valid status transitions
        valid_transitions = {
            "initialized": ["researching", "failed"],
            "researching": ["verifying", "failed"],
            "verifying": ["approved", "correcting", "escalated", "failed"],
            "correcting": ["researching", "escalated", "failed"],
            "approved": [],  # Terminal state
            "escalated": [],  # Terminal state  
            "failed": []  # Terminal state
        }
        
        # For new workflows, any status is valid
        # This validator mainly documents expected transitions
        return v
    
    @validator('resource_usage')
