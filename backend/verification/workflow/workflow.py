...
class VerificationWorkflow:
    ...
    def __init__(self,
        ...
        self._logger = setup_verification_logger("backend.verification.workflow", config)
    ...
    def _execute_research_node(self, state: WorkflowState) -> WorkflowState:
        ...
        with tracer.start_span_sync(
            ...
            try:
                ...
                if not state.get("draft_response", "").strip():
                    fallback_builder = getattr(
                        self.research_agent, "_build_grounded_fallback", None
                    )
                    if callable(fallback_builder):
                        state["draft_response"] = fallback_builder(objection)
                        state["tools_used"] = ["internal_db_search"]
                        state["research_reasoning"] = (
                            "Research agent returned an empty draft; "
                            "used catalog-grounded fallback."
                        )
                ...
                if callable(fallback_builder):
                    state["draft_response"] = fallback_builder(
                        state.get("objection_text", "")
                    )
                    state["tools_used"] = ["internal_db_search"]
                    state["research_reasoning"] = (
                        f"Research agent failed ({type(e).__name__}); "
                        "used catalog-grounded fallback."
                    )
                    state["workflow_status"] = "researching"
                    state["execution_log"].append(
                        ExecutionStep(
                            ...
                            execution_time=execution_time,
                            status=ExecutionStatus.SUCCESS,
                            input_summary="research request with internal fallback",
                            output_summary=state["draft_response"][:100],
                            metrics={
                                "degraded": True,
                                "error_type": type(e).__name__,
                                "correlation_id": correlation_id,
